...
export const generateTikTokHashtags = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ groups: HashtagGroup[]; isAI: boolean }> => {
    ...
    const limit = await checkRateLimit("tiktok-hashtag", { max: 10, windowSeconds: 60 });
    if (!limit.ok) {
    ...
    try {
      ...
      const result = await generateText({
        ...
        system: `You are a TikTok hashtag strategist. Generate hashtag groups for a TikTok video based on the topic.

Return ONLY a JSON object with this exact shape, no markdown fences:
{
  "groups": [
    { "label": "Niche & specific", "hint": "Lower competition, higher intent — the tags most likely to reach the right viewers.", "tags": ["tag1", "tag2", ...] },
    { "label": "Broad & high reach", "hint": "Big FYP tags that add discovery volume on top of your niche set.", "tags": [...] },
    { "label": "Long-tail & community", "hint": "Tags your audience actually follows and browses week to week.", "tags": [...] },
    { "label": "Engagement boosters", "hint": "Save- and share-friendly tags for tutorials, tips, and storytime videos.", "tags": [...] }
  ]
}

RULES:
- Niche & specific: 10 tags, lower competition, high intent
- Broad & high reach: 8 tags, big FYP tags
- Long-tail & community: 8 tags, community/niche tags
- Engagement boosters: 6 tags, save/share/tutorial tags
- All tags as single words or joined words, no # prefix
- Tags must be relevant to the topic
- No duplicates across groups`,
        prompt: `Generate TikTok hashtag groups for this video topic: "${data.topic}"`,
        temperature: 0.7,
      });
