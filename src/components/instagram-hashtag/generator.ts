...
function titleCasePairs(words: string[]) {
  const pairs: string[] = [];
...
export function generateHashtags(input: string): HashtagGroup[] {
  ...
  const longTail = Array.from(
    new Set([
      ...
      ...words.map((w) => `${w}ofinstagram`),
      ...words.map((w) => `${w}2026`),
      ...titleCasePairs(words).map((w) => `${w}guide`),
    ]),
  ).slice(0, 40);
  ...
  return [
    {
      ...
      hint: "Lower competition, higher intent — the tags most likely to reach real buyers.",
      tags: pick(niche.length ? niche : COMMUNITY, 10, seed),
    },
    {
      label: "Broad & high reach",
      hint: "Big, busy tags that add discovery volume on top of your niche set.",
    ...
    },
    {
      ...
      hint: "Save- and share-friendly tags for educational or how-to posts.",
